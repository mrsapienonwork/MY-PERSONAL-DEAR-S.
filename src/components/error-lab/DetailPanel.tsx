...
export function DetailPanel({ errorId, onClose }: DetailPanelProps) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: 20 }}
      className="flex flex-col h-full bg-black/80 sm:bg-black/60 border border-white/10 rounded-2xl overflow-hidden shadow-2xl relative"
    >
      <div className="flex flex-col p-4 sm:p-6 border-b border-white/5 gap-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {errorLocal.status === 'Resolved' ? (
              <button
                ...
                className="px-3 py-1 text-xs border border-white/10 rounded-md hover:bg-white/5 text-gray-400 transition-colors whitespace-nowrap"
              >
                Mark Unresolved
              </button>
            ) : (
              <button
                onClick={handleToggleStatus}
                className="px-3 py-1 text-xs border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 rounded-md hover:bg-emerald-500/20 transition-colors flex items-center gap-1.5 shadow-[0_0_15px_rgba(16,185,129,0.15)] whitespace-nowrap"
              >
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            {!isEditing && (
              <button onClick={() => setIsEditing(true)} className="p-2 w-10 h-10 flex items-center justify-center text-gray-400 hover:text-white rounded-md hover:bg-white/5 transition-colors">
                <Edit2 className="w-5 h-5" />
              </button>
            )}
            {!showDeleteConfirm && (
              <button onClick={() => setShowDeleteConfirm(true)} className="p-2 w-10 h-10 flex items-center justify-center text-rose-400 hover:text-rose-300 rounded-md hover:bg-rose-500/10 transition-colors">
                <Trash2 className="w-5 h-5" />
              </button>
            )}
            <div className="w-px h-6 bg-white/10 mx-1"></div>
            <button onClick={onClose} className="p-2 w-10 h-10 flex items-center justify-center text-gray-400 hover:text-white rounded-md hover:bg-white/5 transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        {showDeleteConfirm && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-rose-500/10 p-4 rounded-xl border border-rose-500/20 w-full mt-2">
            <span className="text-sm text-rose-300 font-medium">Delete this error?</span>
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 sm:flex-none text-sm text-gray-300 hover:text-white px-4 py-2 bg-black/40 rounded-lg border border-white/5 transition-colors">Cancel</button>
              <button onClick={handleDelete} className="flex-1 sm:flex-none text-sm bg-rose-500 text-white px-4 py-2 rounded-lg hover:bg-rose-400 transition-colors">Yes, delete</button>
            </div>
          </div>
        )}
      </div>

      <div className="flex-1 overflow-y-auto p-4 sm:p-6 pb-12 sm:pb-6 space-y-6 custom-scrollbar text-sm w-full">
        ...
        <div>
          ...
          {isEditing ? (
            <div className="flex flex-col sm:flex-row gap-3 mt-2">
              <select 
                ...
                className="bg-white/5 border border-white/10 rounded-lg p-2 text-sm sm:text-xs text-gray-300 outline-none w-full sm:w-auto flex-1"
              >
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4">
            ...
            <div className="flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded-md bg-white/5 text-gray-400 whitespace-nowrap">
              <History className="w-3.5 h-3.5 shrink-0" />
              Reviewed: {errorLocal.reviewCount} times
            </div>
            {errorLocal.isRepeated && (
              <div className="px-2 py-1 text-xs font-medium rounded-md bg-rose-500/20 text-rose-300 whitespace-nowrap">
                Repeated
              </div>
            )}
          </div>
        ...
        {/* Editing Selectors */}
        {isEditing && (
          <div className="space-y-4 bg-white/[0.02] p-3 sm:p-4 rounded-xl border border-white/5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 pt-2 border-t border-white/5 mt-2">
              <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-300">
                <input 
                  type="checkbox" 
                  checked={errorLocal.isRepeated}
                  onChange={e => setErrorLocal({...errorLocal, isRepeated: e.target.checked})}
                  className="w-4 h-4 rounded border-white/20 bg-black/40 text-violet-500 focus:ring-violet-500/50 shrink-0"
                />
      ...
      {isEditing && (
        <div className="p-4 border-t border-white/5 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 bg-black/40">
          <button
            onClick={() => { setIsEditing(false); setErrorLocal(originalError); }}
            className="w-full sm:w-auto px-4 py-2 rounded-lg font-medium text-gray-400 hover:text-white bg-white/5 sm:bg-transparent hover:bg-white/10 sm:hover:bg-white/5 transition-colors flex items-center justify-center"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-lg font-medium transition-colors"
          >
