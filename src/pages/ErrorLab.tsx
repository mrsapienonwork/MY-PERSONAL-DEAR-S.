  ...
  return (
    <div className="h-full flex flex-col font-sans">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-xl text-sm font-medium transition-colors shadow-sm w-full sm:w-auto"
          >
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-6">
                ...
                <div key={group.name} className="space-y-3">
                  ...
                  {group.items.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {group.items.slice(0, 100).map(error => (
                        <button
                          key={error.id}
                          ...
                          className={`w-full text-left p-4 rounded-xl border transition-colors ${
                            ...
                            : 'bg-black/60 sm:bg-black/40 border-white/5 hover:border-white/10 hover:bg-black/80 sm:hover:bg-black/60'
                          }`}
                        >
        ...
        <div className="w-full lg:w-2/3 h-auto lg:h-full min-h-[400px] lg:min-h-0 relative flex flex-col">
           <AnimatePresence mode="wait">
             {isCapturing ? (
               <motion.div key="capture" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex-1">
                 <CapturePanel onClose={() => setIsCapturing(false)} />
               </motion.div>
             ) : selectedErrorId ? (
               <motion.div key={selectedErrorId} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex-1">
                 <DetailPanel errorId={selectedErrorId} onClose={() => setSelectedErrorId(null)} />
               </motion.div>
             ) : (
               <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex-1 flex items-center justify-center bg-black/20 border border-white/5 rounded-2xl p-6 text-center">
