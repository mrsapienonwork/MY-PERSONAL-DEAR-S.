...
function StatCard({ title, value, highlight, colorize }: { title: string, value: string | number, highlight?: boolean, colorize?: boolean }) {
  ...
  return (
    <div 
      className={`p-5 md:p-6 rounded-2xl ${highlight ? 'glass-panel bg-violet-900/20 border border-violet-500/50 shadow-lg' : 'glass-panel border border-white/5'} flex flex-col justify-center transition-colors duration-150 hover:scale-[1.02] hover:bg-white/[0.04]`}
    >
...
function MiniStat({ title, value, color }: { title: string, value: string | number, color: string }) {
    return (
        <div 
          className="p-3 sm:p-4 rounded-xl border border-white/5 glass-panel flex gap-2 sm:gap-3 items-center justify-between shadow-sm transition-colors duration-150 hover:scale-[1.02] hover:bg-white/[0.04]"
        >
