...
// Subcomponent to handle high-frequency timer renders without re-rendering parent
function TimerDisplay({
  ...
  const [displayTime, setDisplayTime] = useState(initialRemainingTime);

  useEffect(() => {
    setDisplayTime(initialRemainingTime);
  }, [initialRemainingTime, isRunning, expectedEndTime]);
  ...
  }, [isRunning, expectedEndTime, onComplete]);
...
export function FocusMode() {
  ...
  // Main timer loop removed, now handled by TimerDisplay subcomponent

  // Use useCallback so TimerDisplay doesn't retrigger unnecessarily
  const handlePhaseComplete = React.useCallback(() => {
    // Current phase is done
    setState(prev => {
      const isFocus = prev.activePhase === 'deep_focus';
  ...
  const glassCardClasses = "bg-[rgba(255,255,255,0.08)]  rounded-[20px] border border-[rgba(255,255,255,0.15)] shadow-lg";
  const glassButtonClasses = "bg-[rgba(255,255,255,0.1)]  border border-[rgba(255,255,255,0.2)] hover:bg-[rgba(120,80,255,0.3)] transition-colors duration-150 text-white";
      ...
      <div className="flex flex-col h-full space-y-6 pb-20 md:pb-safe text-white relative z-0">
        ...
        {randomLetter && (
          <motion.div 
            ...
            className="flex flex-col items-center justify-center p-8 md:p-12 rounded-[24px] border border-white/[0.08] bg-[rgba(255,255,255,0.03)]  shadow-xl text-center relative overflow-hidden group mb-2"
          >
            ...
            <div className="relative z-10 flex flex-col items-center max-w-3xl mx-auto">
              ...
              <div className="flex items-center gap-4">
                ...
                <NavLink 
                  to="/letters" 
                  className="text-xs font-medium text-white/40 hover:text-white transition-colors flex items-center gap-1.5 opacity-0 group-hover:opacity-100 bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-full border border-white/5"
                >
                  <Mail size={12} />
                  View all
                </NavLink>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 flex-1">
          ...
          <div className="lg:col-span-7 xl:col-span-8 flex flex-col gap-6">
            <motion.div 
              layout
              className="flex flex-col items-center justify-center relative overflow-hidden flex-1 min-h-[400px] text-white rounded-[20px] glass-panel border border-[rgba(255,255,255,0.15)] shadow-lg p-12"
              style={{
            ...
            >
              ...
              <div className="relative z-10 flex flex-col items-center w-full">
                <span className={cn(
                  "px-4 py-1.5 rounded-full text-sm font-semibold tracking-widest mb-8 border ",
                ...
                <div className="font-mono text-7xl sm:text-9xl font-bold tracking-tighter text-white drop-shadow-lg mb-12">
                  <TimerDisplay 
                ...
                <div className="flex items-center gap-6 mb-8">
                  ...
                  >
                    <RotateCcw className="w-6 h-6 group-hover:-rotate-180 transition-transform duration-200" />
                  </button>

                  <button 
                    onClick={toggleTimer}
                    className="w-20 h-20 rounded-full flex items-center justify-center bg-white text-black hover:bg-gray-200 transition-colors hover:scale-105 active:scale-95 shadow-lg"
                  >
