        ...
        <AnimatePresence>
            {scoreToast && (
                <motion.div 
                    ...
                    className="absolute right-0 top-0 px-4 py-2 glass-panel border border-violet-500/30 bg-violet-500/10 rounded-xl shadow-md flex items-center gap-2 z-50 text-violet-300 font-bold text-sm tracking-widest"
                >
                    +15 Discipline Score
                </motion.div>
            )}
        ...
        {data.subjects.map(subject => {
          ...
          return (
            <button
              ...
              className={`relative snap-start px-5 py-2.5 rounded-full whitespace-nowrap text-sm font-semibold transition-colors duration-150 border ${
                isActive 
            ...
            >
              ...
              {isActive && (
                <motion.div 
                  ...
                  className="absolute inset-0 bg-white/10  rounded-full border border-white/20 shadow-sm z-0"
        ...
        <div className="flex-1 w-full space-y-3 z-10">
            ...
            <div className="h-3 glass-panel border border-white/5 rounded-full overflow-hidden">
                <motion.div 
                    ...
                    className="h-full bg-violet-500 shadow-[0_0_10px_rgba(139,92,246,0.3)] relative" 
                />
            </div>
        </div>
      </motion.div>
        ...
        {activeSubjectChapters.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4">
                {activeSubjectChapters.map(chapter => (
                    <div 
                      key={chapter.id} 
                      className={`group glass-panel border ${chapter.status === 'Completed' ? 'border-green-500/30 bg-green-950/5' : 'border-white/5 hover:border-white/10'} rounded-2xl p-5 md:p-6 shadow-sm transition-colors duration-150 hover:shadow-2xl hover:bg-white/[0.04]`}
                    >
                        <div className="flex justify-between items-start">
                            <div className="flex-1 pr-4">
                                <h3 className={`text-lg md:text-xl font-bold tracking-tight transition-colors duration-150 ${chapter.status === 'Completed' ? 'text-gray-500 line-through decoration-gray-500/50' : 'text-white'}`}>{chapter.name}</h3>
                                <div className="flex flex-wrap items-center gap-2 mt-3">
                                    ...
                                    {chapter.status === 'Completed' && (
                                        <span 
                                          className="text-[10px] px-2.5 py-1 rounded-md font-bold tracking-widest uppercase bg-green-500/10 text-green-400 border border-green-500/20 animate-in zoom-in duration-150"
                                        >
                                            Completed
                                        </span>
                                    )}
                            ...
                            <button onClick={() => deleteChapter(chapter.id)} className="p-2.5 rounded-xl text-gray-500 hover:text-red-400 hover:bg-red-500/10 md:opacity-0 md:group-hover:opacity-100 transition-colors flex-shrink-0 border border-transparent hover:border-red-500/20">
...
function TaskToggle({ label, done, onClick }: { label: string, done: boolean, onClick: () => void }) {
    return (
        <button 
            onClick={onClick}
            className={`w-full flex items-center justify-start gap-2.5 px-3 py-3 md:py-2.5 rounded-xl text-xs font-semibold transition-colors duration-200 border hover:scale-[1.02] active:scale-95 ${
        ...
        >
            <div className={`transition-colors duration-150 ${done ? 'rotate-[360deg] scale-110' : 'rotate-0 scale-100'}`}>
