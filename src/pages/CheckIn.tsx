      ...
      <motion.div variants={slideUpItem} className="glass-panel border border-white/5 rounded-2xl p-5 sm:p-6 md:p-8 space-y-6 sm:space-y-8 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2 min-w-0">
            ...
            <input 
              ...
              className="w-full glass-input border border-white/5 rounded-xl px-4 py-3.5 text-white text-lg outline-none focus:border-violet-500 focus:bg-white/[0.03] transition-colors"
              placeholder="0.0"
            />
          </div>
          <div className="space-y-2 min-w-0">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full glass-input border border-white/5 rounded-xl pl-4 pr-12 py-3.5 text-white text-lg outline-none focus:border-violet-500 focus:bg-white/[0.03] transition-colors"
                placeholder="Finish chapter 4..."
              />
              <button 
                onClick={() => setEntry({...entry, targetCompleted: !entry.targetCompleted})}
                className={`absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-lg transition-colors duration-200 active:scale-95 hover:scale-[1.05] ${entry.targetCompleted ? 'text-green-500 bg-green-500/10' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
              >
            ...
            {data.habits.map(habit => {
              ...
              return (
                <button
                  ...
                  className={`relative overflow-hidden flex items-center gap-4 p-4 rounded-xl border text-left transition-colors duration-150 ${
                    isChecked 
                      ? 'bg-violet-900/20 border-violet-500/40 text-white shadow-[0_0_15px_rgba(139,92,246,0.15)] scale-[1.01]' 
                      : 'glass-panel border-white/5 text-gray-400 hover:border-white/15 hover:text-gray-200 hover:scale-[1.01]'
                  }`}
                >
                  <div 
                    className={`w-5 h-5 rounded flex-shrink-0 flex items-center justify-center border transition-colors duration-150 ${
                      isChecked ? 'bg-violet-500 border-violet-500' : 'bg-transparent border-white/20'
                    }`}
                  >
                    {isChecked && <CheckCircle className="w-4 h-4 text-white animate-in zoom-in duration-200" />}
                  </div>
        ...
        <div className="space-y-4">
          ...
          <textarea 
            ...
            className="w-full h-32 glass-panel border border-white/5 rounded-xl px-4 py-4 text-white outline-none focus:border-violet-500 focus:bg-white/[0.03] transition-colors resize-none mb-2"
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <input 
              ...
              className="flex-1 min-w-0 glass-input border border-white/5 rounded-xl px-4 py-3.5 text-sm md:text-base text-white outline-none focus:border-violet-500 focus:bg-white/[0.03] transition-colors"
            />
            <input 
              type="text" 
